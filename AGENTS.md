...
**Estado (F0-10):** TypeScript severo, Biome como formato y lint, dependency-cruiser con los
...
Testcontainers, y el mecanismo de semilla (`npm run db:seed`, con el cliente de Prisma armado con
`@prisma/adapter-pg`) — ver *Base de datos*. La app todavía no se conecta a la base; el control de
migraciones y drift de CI es F0-11.
...
- **La semilla** (F0-10, `prisma/seed.ts` + `scripts/db-seed.ts`, en `npm run db:seed`): inserta las
  ...
  ficticias**: clientes, sitios, personas y montos inventados que respeten la forma de los tres
  casos reales (Rio Tinto, MARA, el camión) — el repo es público (P14, regla 1).
- **Todavía no hay** control de migraciones y drift en CI (F0-11).
