...
- **Parseo** (`parsearImporte`, en el dominio porque devuelve un `Importe`, que `app` no puede
  ...
  **punto** opcional y en grupos de tres exactos; hasta **dos** decimales; `-` opcional adelante;
  ...
  `13.720,00`, `13720`, `13720,5`, `-13.720,00`. Se rechazan `13,720.00` (inglés), `13.72`,
...
- **Formato para pantalla** (`USD 13.720,00`) en `src/app/formato/importe.ts`: es presentación,
...
|---|---|
| `number` con decimales (`13720.5`) | Coma flotante: `0,1 + 0,2 ≠ 0,3`, y pierde precisión por encima de 2^53 |
...
| TC como decimal con escala fija | Su inverso (`1/1370,5`) no tiene expresión decimal finita: la vuelta ya redondearía antes de convertir |
...
| Parseo que adivina el formato (`13,720.00` como inglés) | Ambiguo: `1,234` sería mil doscientos o uno con 234; se rechaza y el usuario corrige |
