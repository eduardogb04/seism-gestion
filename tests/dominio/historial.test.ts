  ...
  describe("lectura de los eventos", () => {
    ...
    it("fechaDe devuelve la primera entrada al estado, y null si nunca pasó", () => {
      const historial = armar([
        ["enviado", "2026-08-20"],
        ["rechazado", "2026-08-22"],
        ["borrador", "2026-08-25"],
      ]);

      expect(ciclo.fechaDe(historial, "borrador")).toBe("2026-08-14");
      expect(ciclo.fechaDe(historial, "enviado")).toBe("2026-08-20");
    ...
    it("diasEntre es antisimétrico", () => {
      const historial = armar([["enviado", "2026-08-15"]]);
...
/** Historial que arranca en `borrador` el 14/08 (ficticia) y sigue los pasos dados. */
function armar(
  pasos: readonly (readonly [Estado, string])[],
): Historial<Estado> {
  let historial = ciclo.crear("borrador", marcaEn("2026-08-14"));
